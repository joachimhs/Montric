...
/** @scope EurekaJView.EurekaJDataSource.prototype */
{
    ...
    fetch: function(store, query) {
        ...
        if (query === EurekaJView.INSTRUMENTATION_TREE_QUERY) {
            ...
            var jqxhr = $.ajax({ url: '/instrumentationMenu' , type: 'POST', data: JSON.stringify(requestStringJson), processData: false})
            .done(function(dataHashes) {
              /*var ids = dataHashes.map(function(dataHash) {
                var id = dataHash.id;
                delete dataHash.id; // best practise to remove the 'id' from server response data hash
                return id;
              }, this);*/
            	store.loadRecords(EurekaJView.InstrumentationTreeModel, dataHashes.instrumentationMenu);
        ...
        if (query === EurekaJView.LOGGED_IN_USER_QUERY) {
            ...
            SC.Logger.log('recordType: ' + recordType);
            var jqxhr = $.ajax({ url: '/user' , type: 'POST', data: requestStringJson})
              .done(function(dataHashes) {
                /*var ids = dataHashes.map(function(dataHash) {
                  var id = dataHash.id;
                  delete dataHash.id; // best practise to remove the 'id' from server response data hash
            	  ...
            	  SC.Logger.log('Got User: ' + dataHashes.loggedInUser.username + " with role: " + dataHashes.loggedInUser.userRole);
              });
