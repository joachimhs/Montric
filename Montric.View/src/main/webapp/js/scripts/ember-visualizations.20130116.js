...
/**
  ...
  VisualizationView provides a base visualization class for rendering graph axes,
  performing axis and label formatting, and displaying gridlines.

  In any of your views, simply override xFormatter, and yFormatter to provide custom axis formatting.

  VisualizationView is also responsible for redrawing the element on the page when the content changes.
...
/** @scope Ember.VisualizationView.prototype */ {
  ...
  didInsertElement: function() {
    ...
    svg.append("g").attr("class", "x axis").attr("transform", "translate(0," + (height - yMargin) + ")").call(xAxis);

    svg.append("g").attr("class", "y axis").attr("transform", "translate(" + xMargin + ", 0)").call(yAxis);

    svg.insert("g", ":first-child").attr("class", "x grid").attr("transform", "translate(0," + (height - yMargin) + ")").
          call(xAxis.tickSize(-height + (yMargin * 2), 0, 0).tickFormat(""));

    svg.insert("g", ":first-child").attr("class", "y grid").attr("transform", "translate(" + xMargin + ", 0)").
