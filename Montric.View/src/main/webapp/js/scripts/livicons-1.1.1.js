...
;var defname = "livicon",           //default icon name
    defsize = 64,                   //default size in pixels
    defcolor = "original",          //default color value ('original' or #XXX or #XXXXXX or for ex. 'blue' )
    defhovercolor = "#4FAC9D",      //default color on hover (#XXX or #XXXXXX or ex. 'red')
...
//This function creates and animates every icon. Must be first, before Jquery code!
Raphael.fn.createLivicon = function (iconid, iconname, size, color, hovercolor, animate, looped, iteration, customduration, eventtype, icondata, scalefactor, onparent) {
  ...
  wholeicon = paper.rect(0,0,size,size).attr({fill: "#fff", stroke:"none", "stroke-width":0, opacity:0}).toFront();
    ...
    // event is 'hover'
      ...
      if (looped && !morph) { //animation is looped (for morphs loop is disabled - reasonless effect)
        if (spinner) { //for spiners
          ...
          if (hovercolor) {
              console.log('hovercolor trigger');
              console.log(trigger);
            trigger.hover(
              function(){
                for (var j = 0; j < shapesamount; j++) {
...
//jQuery
//prepare the all necessary parameters for creating and animating every icon on a page
(function($){
$(window).load(function() {

/*$(".livicon").each(function(i,el){//add unique ID to icons
  $(el).attr("id", "livicon-"+i);
});

//In this loop we collect the all data of every icon
$(".livicon").each(function(){
  var iconid = $(this).attr("id"),
      customattr = $(this).data(),
      parent = $(this).parent(),
      iconname = customattr.name||customattr.n ? customattr.name||customattr.n : defname,
      size = customattr.size||customattr.s ? customattr.size||customattr.s : defsize,      
      eventtype = customattr.eventtype||customattr.et ? customattr.eventtype||customattr.et : defeventtype,
      scalefactor = size/32;
  ...
  if (customattr.color == "original" || customattr.c == "original") {
    var color = "original";
  } else if (customattr.color||customattr.c) {
    var color = customattr.color||customattr.c;
  } else {
    var color = defcolor;
  };

  if (defanimated) {
    var animate = customattr.animate == false || customattr.a == false ? customattr.animate || customattr.a : defanimated;
  } else {
    var animate = customattr.animate == true || customattr.a == true ? customattr.animate || customattr.a : defanimated;
  };

  if (deflooped) {
    var looped = customattr.loop == false || customattr.l == false ? false : Infinity;
  } else {
    var looped = customattr.loop == true || customattr.l == true ? Infinity : false;
  };

  if (customattr.iteration || customattr.i) {
    if (Math.round(customattr.iteration)>0 || Math.round(customattr.i)>0) {
      var iteration = Math.round(customattr.iteration) || Math.round(customattr.i);
    } else {
      var iteration = false;
    };
  } else {
  ...
  if (customattr.duration || customattr.d) {
    if (Math.round(customattr.duration)>0 || Math.round(customattr.d)>0) {
      var customduration = Math.round(customattr.duration) || Math.round(customattr.d);
    } else {
  ...
  if (defchangecoloronhover) {
    ...
    if (customattr.hovercolor === false || customattr.hc === false || customattr.hovercolor === 0 || customattr.hc === 0) {
      hovercolor = false;
    } else if (customattr.hovercolor === true || customattr.hc === true || customattr.hovercolor === 1 || customattr.hc === 1) {
      hovercolor = defhovercolor;
    } else if (customattr.hovercolor || customattr.hc) {
      hovercolor = customattr.hovercolor||customattr.hc;
    };
  } else {
  ...
  if (defonparent) {
    var onparent = customattr.onparent == false || customattr.op == false ? false : parent;
  } else {
    var onparent = customattr.onparent == true || customattr.op == true ? parent : false;
  };

    console.log('load eventtype: ' + eventtype);
